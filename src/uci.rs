...
pub fn message_loop(mut buffer: VecDeque<String>) {
    ...
    loop {
        ...
        match tokens.as_slice() {
            ...
            ["ponderhit"] => shared.ponderhit.store(1, Ordering::Release),
        ...
        loop {
            ...
            match message.trim_end() {
                ...
                _ => {
                    ...
                    // Pondering commands should always be processed
                    if shared.status.get() != Status::RUNNING {
...
fn go(threads: &mut ThreadPool, settings: &Settings, shared: &Arc<SharedContext>, tokens: &[&str]) {
    ...
    let (limits, is_ponder) = parse_limits(board.side_to_move(), tokens);
    ...
    // Reset ponderhit flag before starting search
    shared.ponderhit.store(0, Ordering::Release);

    // Set status to PONDERING if this is a ponder search, otherwise RUNNING
    if is_ponder {
        shared.status.set(Status::PONDERING);
    } else {
...
fn parse_limits(color: Color, tokens: &[&str]) -> (Limits, bool) {
    // Check for "ponder" at the end of the go command
    let is_ponder = tokens.last() == Some(&"ponder");
    let tokens = if is_ponder { &tokens[..tokens.len()-1] } else { tokens };

    if let ["infinite"] = tokens {
        let limits = Limits::Infinite;
        return if is_ponder { (Limits::Ponder(Box::new(limits)), true) } else { (limits, false) };
    }
    ...
    for chunk in tokens.chunks(2) {
        if let [name, value] = *chunk {
            ...
            match name {
                "depth" if value > 0 => {
                    let limits = Limits::Depth(value as i32);
                    return if is_ponder { (Limits::Ponder(Box::new(limits)), true) } else { (limits, false) };
                }
                "movetime" if value > 0 => {
                    let limits = Limits::Time(value);
                    return if is_ponder { (Limits::Ponder(Box::new(limits)), true) } else { (limits, false) };
                }
                "nodes" if value > 0 => {
                    let limits = Limits::Nodes(value);
                    return if is_ponder { (Limits::Ponder(Box::new(limits)), true) } else { (limits, false) };
                }
