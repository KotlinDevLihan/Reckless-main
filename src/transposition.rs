...
impl InternalEntry {
    ...
    const fn quality(&self, tt_age: u8) -> i32 {
        let bound_bonus = match self.flags.bound() {
            Bound::Exact => 12,
            Bound::Lower | Bound::Upper => 6,
            Bound::None => 0,
        };

        self.depth() + 2 * self.flags.tt_pv() as i32 + bound_bonus - 4 * self.relative_age(tt_age)
    }
}
...
impl TranspositionTable {
    ...
    pub fn read(&self, hash: u64, halfmove_clock: u8, ply: isize) -> Option<Entry> {
        ...
        for entry in &cluster.entries {
            if key == entry.key && entry.depth() != TtDepth::NONE {
                let quality = entry.quality(tt_age);
    ...
    pub fn write(
        ...
        for candidate in &mut cluster.entries {
            ...
            let quality = candidate.quality(tt_age);
        ...
        if !force && key == entry.key {
            let existing_depth = entry.depth();
