...
impl super::Board {
    ...
    fn generate_moves<T: MoveGenerator>(&self, list: &mut MoveList) {
        ...
        let all_threats = self.all_threats();
        ...
        let target = if self.in_check() {
            between(king, self.checkers().lsb()) | self.checkers().lsb().to_bb()
        } else {
            Bitboard::ALL
        };
        ...
        self.collect_unpinned::<T, _>(list, target, knights & !pinned, self.colors(nstm), empty, knight_attacks);
        self.collect_unpinned::<T, _>(list, target, bishops & !pinned, self.colors(nstm), empty, |sq| bishop_attacks(sq, occupancies));
        self.collect_unpinned::<T, _>(list, target, rooks & !pinned, self.colors(nstm), empty, |sq| rook_attacks(sq, occupancies));
        self.collect_unpinned::<T, _>(list, target, queens & !pinned, self.colors(nstm), empty, |sq| queen_attacks(sq, occupancies));

        self.collect_pinned::<T, _>(list, target, bishops & pinned, king, self.colors(nstm), empty, |sq| bishop_attacks(sq, occupancies));
        self.collect_pinned::<T, _>(list, target, rooks & pinned, king, self.colors(nstm), empty, |sq| rook_attacks(sq, occupancies));
        self.collect_pinned::<T, _>(list, target, queens & pinned, king, self.colors(nstm), empty, |sq| queen_attacks(sq, occupancies));
    ...
    fn collect_unpinned<T: MoveGenerator, F: Fn(Square) -> Bitboard>(
        &self, list: &mut MoveList, target: Bitboard, bb: Bitboard, enemy: Bitboard, empty: Bitboard, attacks: F,
    ) {
        for from in bb {
            if T::KIND == Kind::Noisy {
                list.push_setwise(from, attacks(from) & target & enemy, MoveKind::Capture);
            }
            if T::KIND == Kind::Quiet {
                list.push_setwise(from, attacks(from) & target & empty, MoveKind::Normal);
            }
        }
    }

    fn collect_pinned<T: MoveGenerator, F: Fn(Square) -> Bitboard>(
        &self, list: &mut MoveList, target: Bitboard, bb: Bitboard, king: Square, enemy: Bitboard, empty: Bitboard, attacks: F,
    ) {
        for from in bb {
            let pin_mask = ray_pass(king, from);
            if T::KIND == Kind::Noisy {
                list.push_setwise(from, attacks(from) & target & enemy & pin_mask, MoveKind::Capture);
            }
            if T::KIND == Kind::Quiet {
                list.push_setwise(from, attacks(from) & target & empty & pin_mask, MoveKind::Normal);
            }
        }
    }
