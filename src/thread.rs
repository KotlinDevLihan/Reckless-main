...
use crate::{
    board::Board,
    history::{ContinuationCorrectionHistory, ContinuationHistory, CorrectionHistory, CounterMoveTable, NoisyHistory, QuietHistory},
    nnue::Network,
...
impl ThreadData {
    pub fn new(shared: Arc<SharedContext>) -> Self {
        Self {
            ...
            counter_move: CounterMoveTable::default(),
            best_move_changes: 0,
            optimism: [0; 2],
            root_depth: 0,
            root_delta: 0,
    ...
    /// Check if move is a counter-move (good reply to opponent's last move)
    pub fn is_counter_move(&self, ply: isize, mv: Move) -> bool {
        if ply >= 1 {
            let prev_mv = self.stack[ply - 1].mv;
            if prev_mv.is_present() {
                let stm = self.board.side_to_move();
                let opp_piece = self.board.piece_on(prev_mv.to());
                let counter = self.counter_move.get(stm, opp_piece, prev_mv.to());
                return counter == mv;
            }
        }
        false
