...
impl Board {
    ...
    pub fn update_threats(&mut self) {
        ...
        let enemy_pawns = self.colored_pieces(nstm, PieceType::Pawn);
        let enemy_knights = self.colored_pieces(nstm, PieceType::Knight);
        let enemy_bishops = self.colored_pieces(nstm, PieceType::Bishop);
        let enemy_rooks = self.colored_pieces(nstm, PieceType::Rook);
        let enemy_queens = self.colored_pieces(nstm, PieceType::Queen);
        let mut threats = pawn_attacks_setwise(enemy_pawns, nstm);
        self.state.piece_threats[PieceType::Pawn] = threats;

        threats = Bitboard(0);
        for square in enemy_knights {
            threats |= knight_attacks(square);
        }
        self.state.piece_threats[PieceType::Knight] = threats;

        threats = Bitboard(0);
        for square in enemy_bishops {
        ...
        self.state.piece_threats[PieceType::King] = king_attacks(self.king_square(nstm));
